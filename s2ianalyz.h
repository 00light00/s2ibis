/********************************************************************************
*File Name	: s2ianlyz.java
*Description	: This file contains the classes needed to analyze a s2ibis2
*		: component.
...
$Log: s2ianalyz.java,v $
Revision 1.6  2006/03/27 18:11:12  akvarma
1. Changes done to make 100 points in the VI tables (the number of points
has been made a function of the sweep step). Change made by Jayasree Nayar (njy@cypress.com)
2. Removed redundant System.out lines (//debug)
3. Changed numTablePts code to reflect changed method to get VI datapoints.

Revision 1.5  2005/02/24 22:50:06  akvarma
1. Cleanup code
2. Fixed code that chose the subtraction range for pullup and pulldown curves

Revision 1.4  2004/04/17 20:38:53  akvarma
Added comments, modified 'setupVoltages' class by making Vcc, Gnd and sweepstart
as members (fields) of the class. Fixed the problem of not printing the last VI
value in pulldown table.

Revision 1.3  2003/11/24 05:55:27  akvarma
Added series MOSFET functionality.
...
ibisTypMinMax       sweepStart	=new ibisTypMinMax(); //OUT		
ibisTypMinMax       Vcc  =new ibisTypMinMax();      //OUT
ibisTypMinMax       Gnd  =new ibisTypMinMax();      //OUT

public void setupVoltages(
int		    curveType,  //IN 
ibisModel	    model      //IN 
)
{
ibisTypMinMax       pullupRef,
                    ...
                    powerClampRef,
                    gndClampRef;
ibisTypMinMax       zeros	=new ibisTypMinMax();
double              linRange,
                    minOffset,
                    maxOffset;
...
if ( goodies.IS_ECL( model.modelType ) )
    ...
    if ( curveType == ConstantStuff.CURVE_TYPE_POWER_CLAMP )
        ...
        /*        while ( goodies.ROUND( goodies.fabs( sweepRange / sweepStep ) + 1 ) > 
                ConstantStuff.MAX_TABLE_SIZE )
            sweepStep *= 2;
        */
        /* *sweepRange += *sweepStep / 10; */
        }
    ...
    if ( ( curveType == ConstantStuff.CURVE_TYPE_GND_CLAMP ) ||
        ...
        /*        while ( ( goodies.fabs( sweepRange / sweepStep ) + 1 ) > ConstantStuff.MAX_TABLE_SIZE )
            sweepStep *= 2;
        */
        /* *sweepRange += *sweepStep / 10; */
        }
...
//series
        ...
        /*        while ( ( goodies.fabs( ( Vcc.typ - Gnd.typ + 2 * diodeDrop ) / 
                  sweepStep ) + 1 ) > ConstantStuff.MAX_TABLE_SIZE )
            sweepStep *= 2;
        */
        /* *sweepRange += *sweepStep / 10; */
        }
      ...
      while ((goodies.ROUND(sweepRange / sweepStep) >= ConstantStuff.MAX_TABLE_SIZE) || (sweepStep < 0.01))
 ...
 *              
 */ 
class findSupplyPins{			//Container Class
	...
	int findPins (ibisPin currentPin,pinsList pinList, 
		     boolean hasPinMapping) {		      //main method
  ...
  if (hasPinMapping)			    //returns 0 if successful else 1
   ...
   {	
     ...
     if(!(currentPin.pullupRef.equalsIgnoreCase("NC")))
	  ...
	  tempPList.next());
       if(tempPList == null)	
	 {
	 System.out.println("No Power or Ground pin"); // modify for refined use
	  return(1);       
	 }
       else
   ...
   if(tempPList == null)	
     {
     System.out.println("No Power pin for component. Please specify");
     System.out.println(" at least one pin with model name POWER");
      return(1);       
     }
   else
    {
     System.out.println("Power pin for component available. Proceeding further ..");
     pullupPin = tempPList.current;  
     powerClampPin = tempPList.current;  
    } 
/*--------------------------------------------------------------------
...
*-------------------------------------------------------------------*/
   ...
   if(tempPList == null)	
     {
     System.out.println("No GND pin for component. Please specify ");
     System.out.println(" at least one pin with model name GND"); 
      return(1);       
     }
   else
     {
     System.out.println("GND pin for component available. Proceeding further ..");	 
     gndClampPin = tempPList.current;  
     pulldownPin = tempPList.current;  
     }
   }//else
 ...
 */ 
class sortVIData{
public int sortVIData(ibisModel 	model_p,
		      ibisVItable	pullupData_p,
		      ibisVItable	pulldownData_p,
		      ibisVItable	powerClampData_p,
		      ibisVItable	gndClampData_p){
ibisTypMinMax	Vcc 		= new ibisTypMinMax(),
		Gnd 		= new ibisTypMinMax(),
		sweepStart 	= new ibisTypMinMax();
double		sweepRange	= 0,
		sweepStep 	= 0,
		diodeDrop 	= 0;
int 		numLinearPts 	= 0,
...
if(pullupData_p != null)
  ...
  if(goodies.IS_ECL(model_p.modelType))
    ...
    {
    numInnerPts = goodies.ROUND(goodies.fabs(Vcc.typ - Gnd.typ + 
                                             2*diodeDrop)/sweepStep)+1;
    pointsToSkip = goodies.ROUND(goodies.fabs(sweepRange - 
                                (Vcc.typ - Gnd.typ + 2* diodeDrop))/
				2*sweepStep *numLinearPts);    
    }
...
/**Apply the derating percentage */
  ...
  if (model_p.derateVIPct !=0)
    for(i = 0; i < VITableSize; i++)	//changed from numTablePts to VITableSize 
      {
      if(!(goodies.IS_USE_NA(model_p.pullup.VIs[i].i.min)))
        model_p.pullup.VIs[i].i.min -= model_p.pullup.VIs[i].i.min *
				      (model_p.derateVIPct/100);

      if(!(goodies.IS_USE_NA(model_p.pullup.VIs[i].i.max)))
        model_p.pullup.VIs[i].i.max += model_p.pullup.VIs[i].i.max *
				      (model_p.derateVIPct/100);
...
if(pulldownData_p != null)
  ...
  if(goodies.IS_ECL(model_p.modelType))
    ...
    {    
    numInnerPts = goodies.ROUND(goodies.fabs(Vcc.typ - Gnd.typ + 2*diodeDrop)/
                                             sweepStep)+1;					     
    pointsToSkip = goodies.ROUND(goodies.fabs(sweepRange - 
                                (Vcc.typ - Gnd.typ + 2* diodeDrop))/
				2*sweepStep *numLinearPts);    
  ...
  if (model_p.derateVIPct !=0)
    ...
    for(i = 0; i < VITableSize; i++)
      ...
      if(!(goodies.IS_USE_NA(model_p.pulldown.VIs[i].i.min)))
        model_p.pulldown.VIs[i].i.min -= model_p.pulldown.VIs[i].i.min *
				      (model_p.derateVIPct/100);

      if(!(goodies.IS_USE_NA(model_p.pulldown.VIs[i].i.max)))
        model_p.pulldown.VIs[i].i.max += model_p.pulldown.VIs[i].i.max *
				      (model_p.derateVIPct/100);
  ...
  if (model_p.derateVIPct !=0)
    for(i = 0; i < numTablePts; i++)
      ...
      if(!(goodies.IS_USE_NA(model_p.power_clamp.VIs[i].i.min)))
        model_p.power_clamp.VIs[i].i.min -= model_p.power_clamp.VIs[i].i.min*
				      (model_p.derateVIPct/100);

      if(!(goodies.IS_USE_NA(model_p.power_clamp.VIs[i].i.max)))
        model_p.power_clamp.VIs[i].i.max += model_p.power_clamp.VIs[i].i.max*
				      (model_p.derateVIPct/100);
      }
  }
  ...
  if (model_p.derateVIPct !=0)
    for(i = 0; i < numTablePts; i++)
      ...
      if(!(goodies.IS_USE_NA(model_p.gnd_clamp.VIs[i].i.min)))
        model_p.gnd_clamp.VIs[i].i.min -= model_p.gnd_clamp.VIs[i].i.min *
				      (model_p.derateVIPct/100);

      if(!(goodies.IS_USE_NA(model_p.gnd_clamp.VIs[i].i.max)))
        model_p.gnd_clamp.VIs[i].i.max += model_p.gnd_clamp.VIs[i].i.max *
				      (model_p.derateVIPct/100);
      }
  }
...
public int analyzePin(
	ibisPin	enablePin,		//in
	ibisPin	inputPin,		//in
	...
	int 	iterate,		//in
	int	cleanup)throws IOException		//in
{
 int 	result 	= 0;
 int	i 	= 0;
 int 	index 	= 0;
...
//Series 
 ...
 ibisTypMinMax	Vcc 		= new ibisTypMinMax(),
 		VccClamp 	= new ibisTypMinMax(),
		Gnd 		= new ibisTypMinMax(),
		GndClamp 	= new ibisTypMinMax(),
		sweepStart	= new ibisTypMinMax(),
		sweepStart_pc 	= new ibisTypMinMax();
 double		sweepRange 	= 0,
 		sweepRange_pc 	= 0,
		sweepStep 	= 0,
		sweepStep_pc 	= 0,
...
if ( currentPin.seriesModel !=null )
  {
  if ( goodies.THIS_MODEL_NEEDS_SERIES_VI_DATA( currentPin.seriesModel.modelType ) )
      {
      ...
      index2 = 0;
      VdsList1.gototop();		
    while ( ( index2 < ConstantStuff.MAX_SERIES_TABLES ) && ( VdsList1.current != null ) )
        {
	genVICurve1.VIData = VdsList1.current.seriesVITable;
	currentVds = VdsList1.current.V;
      
	result += genVICurve1.generateVICurve(currentPin, enablePin, 
                        	  inputPin, pullupPin, pulldownPin,
                        	  powerClampPin, gndClampPin, Vcc, Gnd,
                        	  VccClamp, GndClamp, sweepStart, sweepRange,
                        	  sweepStep, ConstantStuff.CURVE_TYPE_SERIES_VI, spiceType,
                        	  seriesSpiceFile, spiceCommand, ConstantStuff.ENABLE_OUTPUT,
...
if ( goodies.THIS_MODEL_NEEDS_PULLUP_DATA( currentPin.model.modelType ) )
    ...
    result += genVICurve1.generateVICurve(currentPin, enablePin, 
                              ...
                              powerClampPin, gndClampPin, Vcc, Gnd,
                              VccClamp, GndClamp, sweepStart, sweepRange,
                              sweepStep, ConstantStuff.CURVE_TYPE_PULLUP, spiceType,
                              spiceFile, spiceCommand, ConstantStuff.ENABLE_OUTPUT,
    ...
    if(enablePin !=null)
     {
     result  += genVICurve1.generateVICurve(currentPin, 
                        	 enablePin, inputPin, pullupPin,
                        	 pulldownPin, powerClampPin,
                        	 gndClampPin, Vcc, Gnd, VccClamp,
                        	 GndClamp, sweepStart, sweepRange,
                        	 sweepStep, ConstantStuff.CURVE_TYPE_DISABLED_PULLUP,
                        	 spiceType, spiceFile, spiceCommand,
                        	 ConstantStuff.DISABLE_OUTPUT, ConstantStuff.OUTPUT_HIGH, 
                        	 iterate,cleanup ,0,0);
...
//Changed code to subtract the entire 'clamp curve(power and gnd)' from the pullupData to result in the 
//new pullupData. This is repeated for pullDown curve.

		    i =0;
//debug	    System.out.println("PullupDataSize = "+ pullupData.size);   
...
//debug			System.out.println("PpullupData.VIs[i].i.typ = "+ pullupData.VIs[i].i.typ);
			}
...
if ( goodies.THIS_MODEL_NEEDS_PULLDOWN_DATA( currentPin.model.modelType ) )
    ...
    * won't comment here.
    *---------------------------------------------------------------*/
    setupVoltages1.setupVoltages( ConstantStuff.CURVE_TYPE_GND_CLAMP, currentPin.model);
    ...
    setupVoltages1.setupVoltages( ConstantStuff.CURVE_TYPE_PULLDOWN, currentPin.model);
    ...
    result += genVICurve1.generateVICurve(currentPin, enablePin, 
                               ...
                               powerClampPin, gndClampPin, Vcc, Gnd,
                               VccClamp, GndClamp, sweepStart, sweepRange,
                               sweepStep, ConstantStuff.CURVE_TYPE_PULLDOWN, spiceType,
                               spiceFile, spiceCommand, ConstantStuff.ENABLE_OUTPUT,
    ...
    if ( enablePin != null )
        ...
        result  += genVICurve1.generateVICurve(currentPin, 
                                    ...
                                    ConstantStuff.CURVE_TYPE_DISABLED_PULLDOWN, spiceType,
                                    spiceFile, spiceCommand, ConstantStuff.DISABLE_OUTPUT,
                                    ConstantStuff.OUTPUT_LOW, iterate, cleanup ,0,0);
...
if ( goodies.THIS_MODEL_NEEDS_POWER_CLAMP_DATA( currentPin.model.modelType ) )
    ...
    setupVoltages1.setupVoltages( ConstantStuff.CURVE_TYPE_PULLUP, currentPin.model);
    ...
    setupVoltages1.setupVoltages( ConstantStuff.CURVE_TYPE_POWER_CLAMP, currentPin.model);
    ...
    result += genVICurve1.generateVICurve(currentPin, enablePin, 
                               ...
                               powerClampPin, gndClampPin, Vcc, Gnd,
                               VccClamp, GndClamp, sweepStart, sweepRange,
                               sweepStep, ConstantStuff.CURVE_TYPE_POWER_CLAMP, spiceType,
                               spiceFile, spiceCommand, ConstantStuff.DISABLE_OUTPUT,
                               ConstantStuff.OUTPUT_HIGH, iterate, cleanup ,0,0); 
...
if ( goodies.THIS_MODEL_NEEDS_GND_CLAMP_DATA( currentPin.model.modelType ) )
    ...
    result += genVICurve1.generateVICurve(currentPin, enablePin, 
                               ...
                               powerClampPin, gndClampPin, Vcc, Gnd,
                               VccClamp, GndClamp, sweepStart, sweepRange,
                               sweepStep, ConstantStuff.CURVE_TYPE_GND_CLAMP, spiceType,
                               spiceFile, spiceCommand, ConstantStuff.DISABLE_OUTPUT,
                               ConstantStuff.OUTPUT_LOW, iterate, cleanup ,0,0);
...
if ( goodies.THIS_MODEL_NEEDS_TRANSIENT_DATA( currentPin.model.modelType ) )
    ...
    setupVoltages1.setupVoltages( ConstantStuff.CURVE_TYPE_RISING_RAMP, currentPin.model);
    ...
    result += genRmpData1.generateRampData( currentPin, enablePin, inputPin,
    ...
    index = 0;
    risingWave.gototop();
    while ( ( index < ConstantStuff.MAX_WAVEFORM_TABLES ) && ( risingWave.current != null ) )
        {
	genWaveData1.currentWave = risingWave.current;

        result += genWaveData1.generateWaveData( currentPin, enablePin, 
                                    inputPin, pullupPin, pulldownPin,
                                    powerClampPin, gndClampPin, Vcc, Gnd,
                                    VccClamp, GndClamp, ConstantStuff.CURVE_TYPE_RISING_WAVE,
                                    spiceType, spiceFile, spiceCommand,
    ...
    index = 0;
    fallingWave.gototop();						
    while ( ( index < ConstantStuff.MAX_WAVEFORM_TABLES ) && ( fallingWave.current != null ) )
	{
	genWaveData1.currentWave = fallingWave.current;  
        result += genWaveData1.generateWaveData( currentPin, enablePin, 
                                    ...
                                    powerClampPin, gndClampPin, Vcc, Gnd,
                                    VccClamp, GndClamp,
                                    ConstantStuff.CURVE_TYPE_FALLING_WAVE, spiceType,
...
public int 	analyzeComponent(
	int 		spiceType,
	int 		iterate,
	int		cleanup,
	String 		spiceCommand)throws IOException
{
int 		result	= 0;
ibisPin		pullupPin;
ibisPin		pulldownPin;
...
ibisPin		inputPin	= new ibisPin();
 ...
 for (int i1 = 0; i1 < CLC.cListCont.count; i1++)
   ...
   if(componentIn.pList!=null)
	 ...
	 System.out.println("modelName for above pin "+ currentPin.modelName);
	 enablePin_P = null;
	 inputPin = null;
...
//series comment
//there might be cases when the normal model has been analyzed, but the series model has not 
    	 if((goodies.THIS_PIN_NEEDS_ANALYSIS(currentPin.modelName))&&
    	 (currentPin.model.hasBeenAnalyzed == 0))
    	   {
      	   System.out.println("THIS_PIN_NEEDS_ANALYSIS" );

    	   result=findSupplyPins1.findPins(currentPin,tempPinsList,componentIn.hasPinMapping);
	   ...
	   componentIn.pList.current = currentPin;	
	       ...
	       {
	       enablePin_P = getMatchingPin1.getMatchingPin(currentPin.enablePin, componentIn.pList);

	       componentIn.pList.current = currentPin;	
	       if(enablePin_P ==null) //If it is STILL null, it wasnt in the pin list
		 {
		 System.out.println("Could'nt find enable pin for "+currentPin.pinName); // modify for refined use
		 result++;
		 }
	       }			  //if enable pin != ""
    	     if(!currentPin.inputPin.equalsIgnoreCase(""))
	       {
	       inputPin = getMatchingPin1.getMatchingPin(currentPin.inputPin, componentIn.pList);

	       componentIn.pList.current = currentPin;		
	       if(inputPin ==null) //If it is STILL null, it wasnt in the pin list
		 {
		 System.out.println("Could'nt find input pin for "+currentPin.pinName); // modify for refined use
		 result++;
		 }
	       }			  //if enable pin != ""
	     if(result==0)
	       {
	       System.out.println("start analyzing pin .. "+currentPin.pinName );
	     ...
	     if(result>0)
	       {
	       System.out.println("Error in Analysis for Pin "+currentPin.pinName); // modify for refined use
	       }			  //if (result) 
	     else
     ...
     {
     System.out.println(". Please specify the pin list using the [Pin] Keyword"); // modify for refined use
     result++;
     }
