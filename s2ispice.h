...
min and max process corner, instead of typ. -- Done by Mike LaBonte (milabont@cisco.com)
Commented out a redundant System.out line (//debug)
...
Cleaned up code.
Removed error because of non-creation of .msg files in HSPICE - converted to warning.
...
class setupPwrAndTempCmds{
 ...
 public void setupPwrAndTempCmds(int curveType,
				 int spiceType,
        			 ibisPin powerPin,
        			 ibisPin gndPin,
 ...
 if((curveType==ConstantStuff.CURVE_TYPE_POWER_CLAMP)||
   ...
   if(powerClampPin !=null)
     {
//debug     System.out.println("powerClampPin.spiceNodeName "+powerClampPin.spiceNodeName);
     ...
     if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
       ...
       if((powerPin !=null)&&!(powerClampPin.spiceNodeName
       			      .equalsIgnoreCase(powerPin.spiceNodeName)))
	 {
         if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
   ...
   if(gndClampPin!=null)
     ...
     powerBuffer=powerBuffer+dummyBuffer;
     if((gndPin!=null)&&!(gndClampPin.spiceNodeName
                         .equalsIgnoreCase(gndPin.spiceNodeName)))
       {
       if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
   ...
   if(gndPin!=null)
     {
     if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
       dummyBuffer= "VGNDS2I "+gndPin.spiceNodeName+ 
       			" 0 "+" vsource type=dc dc="+Gnd+"\n";
     else
       dummyBuffer="VGNDS2I "+gndPin.spiceNodeName+ 
       			" 0 "+" DC "+Gnd+"\n";
   ...
   if(powerPin !=null)
     ...
     if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
       ...
       if((powerClampPin !=null)&&!(powerClampPin.spiceNodeName
       				   .equalsIgnoreCase(powerPin.spiceNodeName)))
	 {
         if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
	   dummyBuffer= "VCLMPS2I "+powerClampPin.spiceNodeName+ 
	   		" 0 "+" vsource type=dc dc="+VccClamp+"\n";
         else
	   dummyBuffer="VCLMPS2I "+powerClampPin.spiceNodeName+ 
	   		" 0 "+" DC "+VccClamp+"\n";
     ...
     if(powerClampPin!=null)
       {
       if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
	 powerBuffer= "VCLMPS2I "+powerClampPin.spiceNodeName+ 
	 		" 0 "+" vsource type=dc dc="+VccClamp+"\n";
       else
	 powerBuffer="VCLMPS2I "+powerClampPin.spiceNodeName+ 
	 		" 0 "+" DC "+VccClamp+"\n";      
   ...
   if(gndPin!=null)
     ...
     powerBuffer=powerBuffer+dummyBuffer;
     if((gndClampPin!=null)&&!(gndClampPin.spiceNodeName
     			      .equalsIgnoreCase(gndPin.spiceNodeName)))
       {
       if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
	 dummyBuffer= "VGCLMPS2I "+gndClampPin.spiceNodeName+ 
	 		" 0 "+" vsource type=dc dc="+GndClamp+"\n";
       else
	 dummyBuffer="VGCLMPS2I "+gndClampPin.spiceNodeName+ 
	 		" 0 "+" DC "+GndClamp+"\n";
...
class setupDCSweepCommands{
  ...
  if(spiceType ==ConstantStuff.SPICE_TYPE_SPECTRE)
    ...
    analysisBuffer = "DCsweep dc dev= VOUTS2I param=dc start="+sweepStart+
    		     " stop="+sweepEnd+" step="+sweepStep+" save=selected\n";
    ...
    {
    analysisBuffer = ".DC VOUTS2I "+sweepStart+" "+sweepEnd+" "+sweepStep+" \n";
    if(curveType == ConstantStuff.CURVE_TYPE_SERIES_VI)
...
class setupTranCommands{
  public String setupTranCommands(int spiceType,
				   double simTime,
				   String outputNode){
  String analysisBuffer;
  String dummyBuffer=null;
...
class setupSpiceInputFile{
 ...
 public  int setupSpiceInputFile(
 	 int		 iterate,		 //IN
	 int		 spiceType,		 //IN
...
headerLine2 = "*Spice Deck created by S2IBIS3 Version "+ConstantStuff.VERSION_STRING+
		" \n*North Carolina State University ";
PrintWriter spiceInWriter= new PrintWriter( new BufferedWriter(		  
new FileWriter(spiceInFile_p)));
...
*--------------------------------------------------------------------*/
   if(!(spiceInFile_p.canWrite()))						  
      {
       System.out.println("Unable to open " + spiceInputFile + " for writing" );	  
       return 1;
      }									  
   else
      {
   ...
   *----------------------------------------------------------------*/
	...
	{
	System.out.println("Error while writing header lines " );	  
	spiceInWriter.close();
	return 1;
	}
   /*-----------------------------------------------------------------
   ...
   *----------------------------------------------------------------*/
	  ...
	  {
	  System.out.println("Unable to open " + spiceFile + " for reading" );
	  spiceInWriter.close();
	  return 1;								      
	  }
	  ...
	  *------------------------------------------------------------*/
	  ...
	  if(cnt != 0)
	    ...
	    if(!(firstArg.equalsIgnoreCase(".end")) && !(firstArg.equals("*")))
	     spiceInWriter.println(lineBuffer);
	     if (spiceInWriter.checkError())
	       {
	       System.out.println("Error copying "+ spiceFile);
	       spiceInWriter.close();
	       tempReader.close();
	       return 1;
	       }
      ...
      try{  
  	    ...
  	    System.out.println("Error "+ e.toString()+" while closing "+ spiceFile);
  	    spiceInWriter.close();
  	    return 1;
      }
   /*-----------------------------------------------------------------
   ...
   *----------------------------------------------------------------*/
      if (!((modelFile.equals(""))||(modelFile.equalsIgnoreCase("NA"))))	//fixed 03/13/04
	 {
	   tempFile_p = new File(modelFile);
	
	   if(!(tempFile_p.canRead()))				       
	       {
	       System.out.println("Unable to open " + modelFile + " for reading" );	   
	       spiceInWriter.close();
	       return 1;
	       }
//	   BufferedReader 
	   ...
	   while((lineBuffer1 = tempReader.readLine()) !=null)
	       ...
	       if (spiceInWriter.checkError())
		  {
		  System.out.println("Error copying "+ modelFile);
		  spiceInWriter.close();
		  tempReader.close();
		  return 1;
		  }
...
*/
   /*-----------------------------------------------------------------
   *   Next, copy the external spice command file to the spice input file.  
   *   However,we only want to do this if a model file has been specified.
   *----------------------------------------------------------------*/
      if (!((extSpiceCmdFile.equals(""))||(extSpiceCmdFile.equalsIgnoreCase("NA"))))
	 {
	   tempFile_p = new File(extSpiceCmdFile);
	
	   if(!(tempFile_p.canRead()))				       
	       {
	       System.out.println("Unable to open " + extSpiceCmdFile + " for reading" );	   
	       spiceInWriter.close();
	       return 1;
	       }
//	   BufferedReader 
	   ...
	   while((lineBuffer1 = tempReader.readLine()) !=null)
	       ...
	       if (spiceInWriter.checkError())
		  {
		  System.out.println("Error copying "+ extSpiceCmdFile);
		  spiceInWriter.close();
		  tempReader.close();
		  return 1;
		  }
	 ...
	 if (spiceInWriter.checkError())
	    {
	    System.out.println("Error while writing simulator language in file "
	    			+ spiceInFile_p);
	    spiceInWriter.close();
	    return 1;
	    }
      ...
      *----------------------------------------------------------------*/
      ...
      if (spiceInWriter.checkError())
	 {
	 System.out.println("Error while writing load to file "+ spiceInFile_p);
	 spiceInWriter.close();
	 return 1;
	 }
      /*-----------------------------------------------------------------
      ...
      *----------------------------------------------------------------*/
      ...
      if (spiceInWriter.checkError())
	 {
	 System.out.println("Error adding power sources to file "+ spiceInFile_p);
	 spiceInWriter.close();
	 return 1;
	 }
       /*-----------------------------------------------------------------
      ...
      if (spiceInWriter.checkError())
	 {
	 System.out.println("Error adding input stimulus to file "+ spiceInFile_p);
	 spiceInWriter.close();
	 return 1;
	 }
      /*-----------------------------------------------------------------
      ...
      *----------------------------------------------------------------*/
      ...
      if (spiceInWriter.checkError())
	 {
	 System.out.println("Error adding Spice options to file "+ spiceInFile_p);
	 spiceInWriter.close();
	 return 1;
	 }
      /*-----------------------------------------------------------------
      ...
      *----------------------------------------------------------------*/
      ...
      if (spiceInWriter.checkError())
	 {
	 System.out.println("Error adding Spice analysis command to file "+ spiceInFile_p);
	 spiceInWriter.close();
	 return 1;
	 }
      /*-----------------------------------------------------------------
	 ...
	 if (spiceInWriter.checkError())
	    {
	    System.out.println("Error completing file "+ spiceInFile_p);
	    spiceInWriter.close();
	    return 1;
	    }
	 }
...
System.out.println("Exception while setting file names (setupSpiceInputFile): "
		   +ex.toString()+": ");
ex.printStackTrace();
}
...
class setPinTran{
  ...
  public String setPinTran(ibisPin pin,
                       int pinType, 
  		       int outputRising,
		       String nodeLabel, 
		       int analyze_case,
  ...
  double Vil=0, Vih=0,risetime=0,falltime=0,pulsewidth=0,period=0;
  String sourceName;
...
period = 2*(risetime+falltime+pulsewidth);
  ...
  if((pinType == ConstantStuff.MODEL_POLARITY_INVERTING)||
     ...
     {
     if(outputRising==1)
       pinBuffer = s2iString.tranSrcString(spiceType,sourceName, pin.spiceNodeName,"0", 
       				     Vih, Vil, risetime, falltime, pulsewidth, period);
     else
       pinBuffer = s2iString.tranSrcString(spiceType,sourceName, pin.spiceNodeName,"0",
       				     Vil, Vih, risetime, falltime, pulsewidth, period);
     }  
  else
     {
     if(outputRising==1)
...
class setPinDC{
  public String setPinDC(ibisPin pin,
                       int pinType, 
  		       int outputActive,
		       String nodeLabel, 
		       int analyze_case,
  ...
  if((pinType == ConstantStuff.MODEL_POLARITY_INVERTING)||
     ...
     if(outputActive==1)
       ...
       if(nodeLabel.equalsIgnoreCase("gate"))
         {
	 if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
           pinBuffer = sourceName+" "+"Vg"+" 0 vsource type=dc dc= "+Vil;
	 else
         ...
         {
	 if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
           pinBuffer = sourceName+" "+pin.spiceNodeName+" 0 vsource type=dc dc= "+Vil;
	 else
       ...
       if(nodeLabel.equalsIgnoreCase("gate"))
         {
	 if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
           pinBuffer = sourceName+" "+"Vg"+" 0 vsource type=dc dc= "+Vih;
	 else
         ...
         {       
	 if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
           pinBuffer = sourceName+" "+pin.spiceNodeName+" 0 vsource type=dc dc= "+Vih;
	 else
...
class callSpice{
  public int callSpice(int iterate,
  ...
  if((iterate!=1) ||!(tempFile_p.canRead()))
    {
    System.out.println("Starting "+s2iString.spiceNameString[spiceType]+
	...
	if((status !=ConstantStuff.SPICE_GOOD_EXIT_CODE)||(pathSep.charAt(0) == ';'))
	  {
	  return 1;
	  }
	else
	...
	System.out.println("Exception while running Spice: "+ex.toString());
	}
    }
  else
    {
    System.out.println("[iterate] set and file "+spiceOutFile+" exists");
    System.out.println("Skipping Run");
    } 
  return 0;   
...
***************************************************************/
class checkForConvergence{
public int checkForConvergence(
                         int spiceType,		//IN
...
*/
 ...
 try
 {
  ...
  while((lineBuffer = tempReader.readLine()) !=null)
    {
    convergenceOccur = lineBuffer.indexOf(s2iString.convergenceMarker[spiceType]);
    if(convergenceOccur != -1 )
      {
      System.out.println("Found '"+s2iString.convergenceMarker[spiceType]+"' in "+tempFile_p);
      tempReader.close();
      return 1;	  
      }
    }
    ...
    System.out.println("Error "+ e1.toString());//+" while closing "+ spiceOutFile);
    return 1;
 }
  ...
  if(tempFile_p.canRead())
	...
	while((lineBuffer = tempReader.readLine()) !=null)
	  ...
	  abortOccur = lineBuffer.indexOf(s2iString.abortMarker[spiceType]);
   	  if(abortOccur != -1 )
	    {
	    System.out.println("Found "+s2iString.abortMarker[spiceType]+" in "+tempFile_p);
	    tempReader.close();
	    return 1;	  
	    }
	  }
	...
	System.out.println("Error "+ e.toString()+" while closing "+ spiceOutFile);
	return 1;
      }
    }
  else
    {  
    System.out.println("Unable to open " + tempFile_p + " for reading" );
    return 1;
    }
 }
 catch(IOException e1){
    System.out.println("Error "+ e1.toString());
    System.out.println("Error in running "+s2iString.spiceNameString[spiceType]);
    System.exit(1);
 }
...
try
  {
   ...
   if(tempFile_p.canRead())
	 ...
	 System.out.println("Error "+ e.toString()+" while closing "+ spiceMsgFile);
	 return 1;
       }
     }
  }
  catch(IOException e1){
      System.out.println("Warning ... "+ e1.toString());//+" while closing "+ spiceOutFile);
  }    
return 0;     
...
double sweepStart, sweepEnd;
 public int runSpiceAgain(int curveType, int spiceType,  
 			  double sweepStep, String spiceInFile,String spiceOutFile,
			  String spiceMsgFile,int iterate, String headerLine, String spiceCommand,
			  String spiceFile,String modelFileTyp, String extSpiceCmdFile, String loadBuffer,
 ...
 *--------------------------------------------------------------------*/
 ...
 System.out.println("Current sweepStart = "+bdSS.setScale(2,BigDecimal.ROUND_HALF_UP));
 bdSE = new BigDecimal(sweepEnd); 
 System.out.println("Current sweepEnd = "+bdSE.setScale(2,BigDecimal.ROUND_HALF_UP));
 System.out.println("Would you like to specifiy a different sweep range? [y/n] ");
 ...
 if((yes_no.equalsIgnoreCase("y"))||(yes_no.equalsIgnoreCase("yes"))||(yes_no.equalsIgnoreCase("")))
   ...
   if(!(user_in_string.equalsIgnoreCase("")))
     ...
     try{
        ...
        System.out.println("Faulty input - assuming default value");
        newSweepStart = 0;        
        }
     }	
   else
   ...
   System.out.print("New sweepEnd (default:"+bdSE.setScale(2,BigDecimal.ROUND_HALF_UP)+") :");
   user_in_string = user_in.readLine();
   if(!(user_in_string.equalsIgnoreCase("")))
     {
     try{     
   ...
   System.out.println("New sweepStart = "+bdSS.setScale(2,BigDecimal.ROUND_HALF_UP));
   System.out.println("New sweepEnd = "+bdSE.setScale(2,BigDecimal.ROUND_HALF_UP));
   ...
   if(setSpInFile.setupSpiceInputFile(iterate, spiceType,spiceInFile,headerLine,
     ...
     {
     System.out.println("Couldn't set up Spice File for "+
...
//callSpice returns 1(error)     
   if(callSpice1.callSpice(iterate, spiceCommand,spiceType, spiceInFile,
	 ...
	 if(chkConvergence.checkForConvergence(spiceType, spiceOutFile) == 1 )
           {
	   System.out.println("Spice run aborted CONVERENCE ISSUE detected.  ");
  	   return 1;
  	   }
         }     
     }      
   ...
   {
   System.out.println("You entered '"+yes_no+"'");
   System.out.println("Error in executing "+s2iString.spiceNameString[spiceType]);
...
class  getSpiceVIData{
	public int getSpiceVIData(
	    VIDataContainer VIC,	    //IN
	    int 	    tableSize,      //IN
	    int 	    spiceType,      //IN
	    String	    spiceOutFile,   //IN
	    String	    command){	    //IN    
	...
	int 	rowIndex =0,
		...
		numRead = 0;
                boolean flag = false;
                boolean flag_eldo = true;		//changed for beta0.4	
                double temp_double=0;
if(!(outFile_p.canRead()))
    {
    System.out.println("Unable to open " + spiceOutFile + " for reading" ); 
    return 1;
    }
...
try{	
	...
	if(lineBuffer.indexOf(s2iString.VIDataBeginMarker[spiceType])<0)
	    {
	    System.out.println("Data begin marker '"+
	    			s2iString.VIDataBeginMarker[spiceType]+"'");
	    System.out.println("not found in output file " + spiceOutFile ); 
	    SpiceReader.close();
	    return 1;   
	    }
...
System.out.println("Exception while collecting spice data from out files (getSpiceVIData): "+ex.toString());
  	 }	
...
public int generateVICurve(
ibisPin      	     currentPin,	//rest are all in
ibisPin              enablePin,
ibisPin              inputPin,
ibisPin              powerPin,
ibisPin              gndPin,
...
ibisTypMinMax        Vcc,
ibisTypMinMax        Gnd,
ibisTypMinMax        VccClamp,
ibisTypMinMax        GndClamp,
ibisTypMinMax        sweepStart,
double               sweepRange,
double               sweepStep,
int                  curveType,
int                  spiceType,
...
System.out.println("CurrentPin is .. "+currentPin.pinName );
...
*----------------------------------------------------*/
System.out.println("Do the TYP run in GenerateVITable ");
...
if(curveType == ConstantStuff.CURVE_TYPE_SERIES_VI)
  ...
  inputBuffer = setPinDC1.setPinDC(inputPin, model.polarity,outputHigh,  				      
  				     "gate", ConstantStuff.TYP_CASE, spiceType);
  if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
  ...
  if(enablePin !=null)
    inputBuffer = setPinDC1.setPinDC(enablePin, model.enable,
  				     enableOutput, 
  				     "ENA", ConstantStuff.TYP_CASE, spiceType);
  else
    inputBuffer = "";
...
if(setSpInFile.setupSpiceInputFile(iterate, spiceType,spiceInFile,headerLine,
 ...
 if(callSpice1.callSpice(iterate, spiceCommand,spiceType, spiceInFile,
   ...
   if ( returnFromAbort == 1 )
       ...
       if(chkConvergence.checkForConvergence(spiceType, spiceOutFile) == 1 )
         {
	 System.out.println("Spice run aborted NON CONVERENCE detected.  ");
	 ...
	 System.out.println("Rerun simulations.. ");
	 }
       else 
         {
	 System.out.println("--- ");
...
// else
  if(VIData1.getSpiceVIData(VICont,numTablePoints,spiceType,spiceOutFile,"typ")==1)
    {
    System.out.println("Curve "+s2iString.curveNameString[curveType]+
                       " not generated.\n");
    return 1;
    }  
if(cleanup == 1)
  ...
  if(!((spiceType ==ConstantStuff.SPICE_TYPE_ELDO)||(spiceType ==ConstantStuff.SPICE_TYPE_HSPICE)))
    if(!msgFile.delete())
    System.out.println("Can't remove file "+ spiceMsgFile);  
  
  if(spiceType ==ConstantStuff.SPICE_TYPE_HSPICE)
    {
    spiceOutFile= s2iString.spiceFileTypPrefix[curveType]+currentPin.pinName+
                  ".st0";
    File outFile2 = new File (spiceOutFile);
    if(!outFile2.delete())
     System.out.println("Can't remove .st0 file "+ spiceOutFile);
    }  
  }
/*----------------------------------------------------
*    Set up for the min run, and do the run
*----------------------------------------------------*/
System.out.println("Do the min run in GenerateVITable ");
...
if(curveType == ConstantStuff.CURVE_TYPE_SERIES_VI)
  {
  inputBuffer = setPinDC1.setPinDC(inputPin, model.polarity,outputHigh,  				      
  				     "gate", ConstantStuff.MIN_CASE, spiceType);
  if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
    inputBuffer = inputBuffer +"\nVDS "+currentPin.pinName+" "+currentPin.seriesPin2name+
                " vsource type=dc dc= "+ Vds+"\n";
  else
...
if(cleanup == 1)
  ...
  if(spiceType ==ConstantStuff.SPICE_TYPE_HSPICE)
    {
    spiceOutFile= s2iString.spiceFileMinPrefix[curveType]+currentPin.pinName+
                  ".st0";
    File outFile2 = new File (spiceOutFile);
    outFile2.delete();
    }  
...
*----------------------------------------------------*/
System.out.println("Do the max run in GenerateVITable ");
...
if(curveType == ConstantStuff.CURVE_TYPE_SERIES_VI)
  {
  inputBuffer = setPinDC1.setPinDC(inputPin, model.polarity,outputHigh,  				      
  				     "gate", ConstantStuff.MAX_CASE, spiceType);
  if(spiceType==ConstantStuff.SPICE_TYPE_SPECTRE)
    inputBuffer = inputBuffer +"\nVDS "+currentPin.pinName+" "+currentPin.seriesPin2name+
                " vsource type=dc dc= "+ Vds+"\n";
  else
...
if(cleanup == 1)
  ...
  if(spiceType ==ConstantStuff.SPICE_TYPE_HSPICE)
    {
    spiceOutFile= s2iString.spiceFileMaxPrefix[curveType]+currentPin.pinName+".st0";
    File outFile2 = new File (spiceOutFile);
    outFile2.delete();
    }  
  }
...
class getSpiceRampData{
  public int getSpiceRampData(ibisModel model,
  			      int curveType,
 			      int spiceType,
			      String spiceOutFile,
			      String command
  ...
  int key1=0;
  boolean flag = false;					    
  boolean flag_eldo = true;		//changed for beta 0.4
  double temp_double=0;
     ...
     if (lineBuffer !=null)
     	 ...
     	 int cnt = st3.countTokens();
     	     ...
     	     */
     	     ...
     	     if ((cnt ==3)&&flag)
     	       {
//   	       As we dont want the first column from the 3 columns..
//   	       ..we skip the temp_double and go to the nextToken.
     	       //convert string to double using Double.parseDouble
     ...
     for(Iterator key=rampData.keySet().iterator(),value = rampData.values().iterator();
     	 key.hasNext();)
       {
       key.next();
     ...
     boolean t20_done = false;
     boolean t80_done = false;
     if(v1 < v0)
     ...
     *-----------------------------------------------------------------------*/
	 ...
	 model.ramp.dt_r.min -= model.ramp.dt_r.min * model.derateRampPct/100;
	 }
       else
         {
	 model.ramp.dv_r.max = v80-v20;
	 model.ramp.dt_r.max  = t80-t20;  	
	 model.ramp.dt_r.max  += model.ramp.dt_r.max * model.derateRampPct/100;	 
	 }
       }
     else
       {
	 ...
	 model.ramp.dt_f.min -= model.ramp.dt_r.min * model.derateRampPct/100;
         }
       else
         {	 
	 model.ramp.dv_f.max = v20 - v80;
	 model.ramp.dt_f.max  = t80 - t20;  	
	 model.ramp.dt_f.max  += model.ramp.dt_r.max * model.derateRampPct/100;	 
         }         
       }
  SpiceReader.close();
}
catch(Exception ex){
System.out.println("Exception while collecting spice data from out files (getSpiceRampData): "
		   +ex.toString());
ex.printStackTrace();
  	 }	 
return 0;	 
...
class generateRampData{
 public int generateRampData(ibisPin 	currentPin,
 			     ibisPin enablePin,
			     ibisPin inputPin,
			     ibisPin powerPin,
			     ibisPin gndPin,
 ...
 *-----------------------------------------------------------------------*/
 if ( ( model_p.modelType == ConstantStuff.MODEL_TYPE_OPEN_DRAIN ) ||
   ...
   dummyBuffer = s2iString.DCSrcString(spiceType,"VTERMS2I", 
 ...
 *-----------------------------------------------------------------------*/
 ...
 setPinDC setPinDC1 = new setPinDC();
 if(enablePin !=null)
   inputBuffer = setPinDC1.setPinDC( enablePin, model_p.enable,ConstantStuff.ENABLE_OUTPUT, 
  				   "ENA", ConstantStuff.TYP_CASE, spiceType);
 else
   inputBuffer = "";
 ...
 if(inputPin != null)
   ...
   dummyBuffer= setPinTran1.setPinTran( inputPin, model_p.polarity, outputState,
	   				 "IN",ConstantStuff.TYP_CASE, spiceType);
   inputBuffer= inputBuffer+"\n"+dummyBuffer;
   }
 ...
 if(cleanup == 1)
  ...
  if(!((spiceType ==ConstantStuff.SPICE_TYPE_ELDO)||(spiceType ==ConstantStuff.SPICE_TYPE_HSPICE)))
   ...
   if(spiceType ==ConstantStuff.SPICE_TYPE_HSPICE)
     {
     spiceOutFile= s2iString.spiceFileTypPrefix[curveType]+currentPin.pinName+".st0";
     File outFile2 = new File (spiceOutFile);
     if(!outFile2.delete())
       System.out.println("Can't remove .st0 file "+ spiceOutFile);

     hspiceIcFile= s2iString.spiceFileTypPrefix[curveType]+currentPin.pinName+".ic";
     File outFile3 = new File (hspiceIcFile);
     outFile3.delete();     
     hspiceIc0File= s2iString.spiceFileTypPrefix[curveType]+currentPin.pinName+".ic0";
 ...
 *-----------------------------------------------------------------------*/
 ...
 if(enablePin !=null)
   inputBuffer = setPinDC1.setPinDC( enablePin, model_p.enable,
   				     ConstantStuff.ENABLE_OUTPUT, 
  				     "ENA", ConstantStuff.MIN_CASE, spiceType);
 else
   inputBuffer = "";
 
 if(inputPin != null)
   {
   dummyBuffer= setPinTran1.setPinTran( inputPin, model_p.polarity, outputState, "IN",
   				  ConstantStuff.MIN_CASE, spiceType);
   inputBuffer= inputBuffer+"\n"+dummyBuffer;
   }
 ...
 if(cleanup == 1)
  ...
  if(!((spiceType ==ConstantStuff.SPICE_TYPE_ELDO)||(spiceType ==ConstantStuff.SPICE_TYPE_HSPICE)))
   ...
   if(spiceType ==ConstantStuff.SPICE_TYPE_HSPICE)
     {
     spiceOutFile= s2iString.spiceFileMinPrefix[curveType]+currentPin.pinName+".st0";
     File outFile2 = new File (spiceOutFile);
     if(!outFile2.delete())
       System.out.println("Can't remove .st0 file "+ spiceOutFile);
     
     hspiceIcFile= s2iString.spiceFileMinPrefix[curveType]+currentPin.pinName+".ic";
     File outFile3 = new File (hspiceIcFile);
     outFile3.delete();     
     hspiceIc0File= s2iString.spiceFileMinPrefix[curveType]+currentPin.pinName+".ic0";
     File outFile4 = new File (hspiceIc0File);
     outFile4.delete();     
     }
  }  
 ...
 *-----------------------------------------------------------------------*/
 ...
 if(enablePin !=null)
   inputBuffer = setPinDC1.setPinDC( enablePin, model_p.enable,
   				     ConstantStuff.ENABLE_OUTPUT, 
  				     "ENA", ConstantStuff.MAX_CASE, spiceType);
 else
   inputBuffer = "";
 ...
 if(inputPin != null)
   {
   dummyBuffer= setPinTran1.setPinTran( inputPin, model_p.polarity, outputState,
   				   "IN",ConstantStuff.MAX_CASE, spiceType);
   inputBuffer= inputBuffer+"\n"+dummyBuffer;
   }
 ...
 if(cleanup == 1)
  ...
  if(!((spiceType ==ConstantStuff.SPICE_TYPE_ELDO)||(spiceType ==ConstantStuff.SPICE_TYPE_HSPICE)))
   ...
   if(spiceType ==ConstantStuff.SPICE_TYPE_HSPICE)
     ...
     hspiceIcFile= s2iString.spiceFileMaxPrefix[curveType]+currentPin.pinName+".ic";
     File outFile3 = new File (hspiceIcFile);
     outFile3.delete();     
     hspiceIc0File= s2iString.spiceFileMaxPrefix[curveType]+currentPin.pinName+".ic0";
     File outFile4 = new File (hspiceIc0File);
     outFile4.delete();     
     }  
  }  
...
class getSpiceWaveData{
 ...
 public int getSpiceWaveData(double 	simTime,
 			     int 	spiceType,
			     String	spiceOutFile,
			     String 	command){
 File outFile_p = new File(spiceOutFile);
 ...
 *----------------------------------------------------------------------*/	
 ...
 if(lineBuffer.indexOf(s2iString.tranDataBeginMarker[spiceType])<0)		       
     {										       
     System.out.println("Data begin marker "+ s2iString.tranDataBeginMarker[spiceType]);
     System.out.println("not found in output file " + spiceOutFile ); 		       
     SpiceReader.close();							       
     return 1;  							   	       
     }
...
//		..we skip the temp_double and go to the nextToken.
 ...
 System.out.println("Exception while collecting spice data from out files (getSpiceWaveData): "
 		    +ex.toString());
  	  }
 return 0;	 
...
class binTranData{
public void binTranData(double t,
			double v,
			double simTime,
			double binTime,
			String command,
...
ibisWaveTable currentWave;					//inout
 public  int generateWaveData(	    ibisPin	currentPin,	//rest in
 				    ibisPin	enablePin,
				    ibisPin	inputPin,
				    ibisPin	powerPin,
				    ibisPin	gndPin,
 ...
 currentWave.waveData = new ibisWaveTableEntry[ConstantStuff.WAVE_POINTS_DEFAULT];
...
*-----------------------------------------------------------------------*/
 ...
 if(!(goodies.IS_USE_NA(currentWave.L_dut)))
   {
   dummyBuffer = s2iString.indString(spiceType, "LDUTS2I", nodeList[nodeIndex],
 ...
 if(!(goodies.IS_USE_NA(currentWave.R_dut)))
   {
   dummyBuffer = s2iString.resString(spiceType, "RDUTS2I", nodeList[nodeIndex],
 ...
 if(!(goodies.IS_USE_NA(currentWave.C_dut)))
   ...
   dummyBuffer = s2iString.capString(spiceType, "CDUTS2I", nodeList[nodeIndex],
   		 "0",currentWave.C_dut);
   loadBuffer = loadBuffer + dummyBuffer;
   }
...
*-----------------------------------------------------------------------*/
 if(!(goodies.IS_USE_NA(currentWave.L_fixture)))
   {   
   dummyBuffer = s2iString.indString(spiceType, "LFIXS2I", nodeList[nodeIndex],
 ...
 if(!(goodies.IS_USE_NA(currentWave.C_fixture)))
   {
   dummyBuffer = s2iString.capString(spiceType, "CFIXS2I", nodeList[nodeIndex],
...
*-----------------------------------------------------------------------*/
   dummyBuffer = s2iString.resString(spiceType, "RFIXS2I", nodeList[nodeIndex],
...
//initializing the first element of the array = 0 here instead of 'getspiceWaveData'
 ...
 *-----------------------------------------------------------------------*/ 
 ...
 if(inputPin != null)
   {
   dummyBuffer= setPinTran1.setPinTran( inputPin, model_p.polarity, outputState,
   				   "IN",ConstantStuff.MIN_CASE, spiceType);
   inputBuffer= inputBuffer+"\n" +dummyBuffer;
   }
...
for(int m = 0; m < 100; m++)
 ...
 VICont.VIData2.VIs[m] = new ibisVItableEntry();
		    ...
		    VI.getSpiceVIData(VICont,100, 4,"pdxout2.out", "max");
   		    ...
   		    for (int b = 0; b<100; b++)
		      {
System.out.println("ARRAY  :"+VICont.VIData2.VIs[b].v+" ,"+VICont.VIData2.VIs[b].i.typ);
System.out.println("ARRAY2 :"+VICont.VIData2.VIs[b].v+" ,"+VICont.VIData2.VIs[b].i.min);
System.out.println("ARRAY3 :"+VICont.VIData2.VIs[b].v+" ,"+VICont.VIData2.VIs[b].i.max);
		      }
		    }
		 catch(Exception ex){
  		System.out.println("Exception2 while setting file names: "+ex.toString());
  		 }
  	 }
	 }
