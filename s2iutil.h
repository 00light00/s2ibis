...
Removed test 'System.out.println' code
...
public ibisPin getMatchingPin(
	...
	pinsList pinList)
 {
  ...
  if(tempPin==null)
   {
    System.out.println("Pin "+ searchName+" Not Found in pin list"); 
   } 
   return(tempPin);
...
class getMatchingModel{
public ibisModel getMatchingModel(String searchName,
...
public int completeDataStructures(ibisGlobal global)
...
public int linkSeriesPinsToSeriesModels()
  {
  ...
  //series models are also part of the models - also the series models name 
  //would be stored in the modelname field
...
public int linkPinsToModels()
  ...
  getMatchingModel getMModel = new getMatchingModel();
	...
	if(currentModel == null)
	  {
	  System.out.println("Please check that you have correctly spelt the ");
	  System.out.println("model name for pin '"+currentPin.pinName+"'");
	  System.out.println(" in the [Pin] section of your input file and ");
	  System.out.println("that you have specified the model using the ");
	  System.out.println("[Model] keyword before the end of component ");
	  System.out.println(currentComponent.component);	  
	  return 1;
	  }
...
public void copyGlobalDataToComponents(ibisGlobal global)
  {
  ibisComponent currentComponent;
...
public void copyComponentDataToModels()
  {
  ibisComponent 	currentComponent;
...
public int setModelDefaults()
  ...
  for (int i = 0; i < ibis.cList.count; i++) 
    ...
    currentComponent.manufacturer = ConstantStuff.MANUFACTURERS_NAME_DEFAULT;
	...
	{
      currentModel.voltageRange.typ=ConstantStuff.VOLTAGE_RANGE_TYP_DEFAULT;
      currentModel.voltageRange.min=ConstantStuff.VOLTAGE_RANGE_MIN_DEFAULT;
      currentModel.voltageRange.max=ConstantStuff.VOLTAGE_RANGE_MAX_DEFAULT;
	}
      if(goodies.IS_USE_NA(currentModel.Rload))
...
public int copyPinMapToPins()
  ...
  for (int i = 0; i < currentComponent.pList.count; i++)  
    ...
    currentPinMap = getMatchingPinMap(currentPin.pinName, 
      				      currentComponent.pmList);
...
public ibisPinMap getMatchingPinMap(String searchName, pinMapList pinMapList)
  {
  pinMapList.gototop();
  ...
  if(tempPinMap==null)
   {
    System.out.println("Pin "+ searchName+" not in pin map");
    System.out.println("Please include the pin in the [Pin Mapping] section ");
    System.out.println("of your  input file or make sure the pin name is not ");
...
public int copySeriesPinToPins()
  {
  ibisSeriesPin 	currentSeriesPin;
  ibisPin		currentPin;
  ...
  for (int i = 0; i < currentComponent.pList.count; i++)  
    ...
    currentSeriesPin = getMatchingSeriesPin(currentPin.pinName, 
      				      currentComponent.spList);
...
public ibisSeriesPin getMatchingSeriesPin(String searchName, seriesPinsList spList)
  {
  ...
  if (spList != null)
    ...
    if(tempSeriesPin==null)
       System.out.println("Pin "+ searchName+" not in Series Pin Map");
    }
...
ibisTOP	ibisHead;
 public void printMain(){
   ...
   try{ 
    ...
    System.out.println("Opening file "+ ibisHead.thisFileName+" for writing ...");
    ...
    *--------------------------------------------------------------------*/
    if(!(outFile.canWrite()))						  
       {
	System.out.println("Unable to open " + outFile + "for writing" );	  
	return;
       }									  
   ...
   System.out.println("Exception while writing ibis output file (printMain): "
                       +ex.toString()+": ");
   ex.printStackTrace();
   }
 }			//method printMain
 public void print_TOP(PrintWriter ibisWriter){
   ...
   " created by s2ibis3 verion "+ConstantStuff.VERSION_STRING);
   ...
   printString(ibisWriter, "[IBIS ver]", 
               ConstantStuff.MAX_LINE_LENGTH,version);   
...
****************************************************************/
 void printComponent(PrintWriter ibisWriter,
 		     ibisComponent component, 
		     int ibisVersion)
 {
...
// Print component Header
 ...
 printString(ibisWriter,"[Component]",ConstantStuff.MAX_LINE_LENGTH,component.component);
 printString(ibisWriter,"[Manufacturer]",ConstantStuff.MAX_LINE_LENGTH,component.manufacturer);
...
/**Model Header */
 ...
 printString(ibisWriter,"[Model]",ConstantStuff.MAX_LINE_LENGTH,model.modelName);   
...
if(!s.equals(""))
  printString(ibisWriter,"Polarity      ",ConstantStuff.MAX_LINE_LENGTH,s);   
...
if(!s.equals(""))
  printString(ibisWriter,"Enable       ",ConstantStuff.MAX_LINE_LENGTH,s);   
...
if((ibisVersion != ConstantStuff.VERSION_ONE_ONE)&&
  ...
  printString(ibisWriter,"[Rpower]     ",ConstantStuff.MAX_LINE_LENGTH,s);   
  }
if((ibisVersion != ConstantStuff.VERSION_ONE_ONE)&&
...
if((ibisVersion != ConstantStuff.VERSION_ONE_ONE)&&
  ...
  printString(ibisWriter,"[Temperature Range]",ConstantStuff.MAX_LINE_LENGTH,s);   
  }
...
if((ibisVersion != ConstantStuff.VERSION_ONE_ONE)&&
 ...
 if(ibisVersion != ConstantStuff.VERSION_ONE_ONE)
   ...
   for(int i = 0; i < VdsList1.count; i++)
     ...
     printString(ibisWriter,"[series MOSFET]",ConstantStuff.MAX_LINE_LENGTH,"");
     numberBuffer = Get_Double_as_String(VdsList1.current.V,"");
     printString(ibisWriter,"Vds = ",ConstantStuff.MAX_LINE_LENGTH,numberBuffer);
   ...
   if((ibisVersion != ConstantStuff.VERSION_ONE_ONE)&&
     ...
     printString(ibisWriter,"[R Series]     ",ConstantStuff.MAX_LINE_LENGTH,s);   
     }
 ...
 if(ibisVersion != ConstantStuff.VERSION_ONE_ONE)
   ...
   fallingWaveL.gototop();
     ...
     printString(ibisWriter,"[Falling Waveform]",ConstantStuff.MAX_LINE_LENGTH,"");
...
****************************************************************/
 ...
 /***Now do lines 2 through to (size - 1). For each line - check to see if it's 
 * above the clamp tolerance. If not, just advance to the next line, and print
 * zeroes if the line before this was printed. If it is above the clamp tolerance, 
 * check which line was printed last. If it wasn't the line directly before the one 
 * we're on now, print zeroes for that line, then print the current line.**/
 for(i = 1; i<size -1; i++)
   {
...
/**Now for the falling case*/
 ...
 printString(ibisWriter,"R_load =",ConstantStuff.MAX_LINE_LENGTH,numberBuffer);
 printString(ibisWriter,"|",ConstantStuff.MAX_LINE_LENGTH,"");
 }
...
*/ 
 String Get_Double_as_String(double number,  String trailStr)
 {
   ...
   if(goodies.IS_USE_NA(number))
	 ...
	 s =bd.setScale(4,BigDecimal.ROUND_HALF_UP) + s2iString.SIsuffixString[i] + trailStr; 
	 expFound = 1;
	 break;
	 }
       else
   ...
   if(goodies.IS_USE_NA(number))
     ...
     for (i = 0; i<9; i++)
       ...
       if(isExponent(number,exponent))
         {
	 bd = new BigDecimal(number/exponent);
	 s =bd.setScale(2,BigDecimal.ROUND_HALF_UP) + s2iString.SIsuffixString[i] + trailStr; 
	 expFound = 1;
	 break;
	 }
       else
 ...
 {
   if((goodies.fabs(number/expon) >=0.1) &&(goodies.fabs(number/expon) < 100))
     return true;
   else
     return false; 
 }
 ...
 void printHeader( PrintWriter ibisWriter, String name, String heading )
 {
   int spacing;
   String spaceFill= " ";
   spacing = (ConstantStuff.MAX_LINE_LENGTH - 18 - (heading.length() +name.length()))/2; 
   for(int i2 = 0; i2<spacing; i2++)
   spaceFill =spaceFill+" ";

  ibisWriter.println("|************************************************************************");
   printString(ibisWriter,"|",ConstantStuff.MAX_LINE_LENGTH,spaceFill+heading+" "+name);
  ibisWriter.println("|************************************************************************");
   printString(ibisWriter,"|",ConstantStuff.MAX_LINE_LENGTH,"");
 ...
 void printString( PrintWriter ibisWriter, String indent,int width, String string )
 {
	 ...
	 for(int i =0; i<cnt; i++)
	   ...
	   if(newString.length() +2*j*8 >= j*width)	   	//added 2*j*8 to fix 80 char error
             {
	     newString = newString +"\n  ";//+" "+st.nextToken();
	     j++;
	     }
